#region concat
#concat - değişken birleştirme
"""
statik ifade iile değişken veya iki tane değişkeni(dinamik ifade) birleştiririz.
okulNumarasi = 271
ad = "Emir"
soyad = "Besi"
sinavNotu = 99
print(okulNumarasi,ad, soyad, sinavNotu)
print("okul numarası", okulNumarasi," olan ", ad, soyad, "isimli öğreninin sınav notu", sinavNotu)
"""
#endregion

#region concat_ornek1
#windows versiyonu 10 python versiyonu 3.9.2
"""
programlamaDili = "python"
os = "10"
ver = "3.92"
print("windows versiyonu " + os + " " + programlamaDili, " versiyonu " + ver)
"""
#endregion

#region concat_ornek2
#istanbul ile danimarka arası 2800 km.
"""
baslangic= "istanbul"
varis = "danimarka"
mesafe = "2800"
print(baslangic + " ile " + varis + " arası " + mesafe + " km.")
"""
#endregion